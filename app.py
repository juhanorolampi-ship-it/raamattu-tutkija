...
# (Kaikki tausta-funktiot pysyvät täysin samoina)
@st.cache_data
...
def luo_sisallysluettelo(aihe, sanamaara, malli, noudata_perusohjetta):
    prompt = f"Olet teologi. Luo yksityiskohtainen sisällysluettelo (outline) noin {sanamaara} sanan opetukselle aiheesta '{aihe}'. Rakenna runko, jossa on johdanto, 3-5 pääkohtaa, ja jokaiseen 2-4 alakohtaa, sekä yhteenveto. Vastaa AINOASTAAN numeroituna listana."
...
def kirjoita_osio(aihe, osion_otsikko, jakeet, sanamaara_osio, malli, noudata_perusohjetta):
    ...
    prompt = f"Olet teologi. Kirjoita yksi kappale laajasta opetuksesta pääaiheella '{aihe}'. Käsiteltävän kappaleen otsikko on: '{osion_otsikko}'. Kirjoita tästä aiheesta syvällinen, noin {sanamaara_osio} sanan osuus. ÄLÄ TOISTA YLLÄ OLEVAA OTSIKKOA VASTAUKSESSASI. Aloita suoraan leipätekstillä. Käytä AINOASTAAN alla annettua KR33/38-lähdemateriaalia ja lainaa keskeiset jakeet sanatarkasti. LÄHDEMATERIAALI:\n{jae_teksti}"
    return tee_api_kutsu(prompt, malli, noudata_perusohjetta)
...
def paranna_tekstin_osaa(koko_teksti, muokattava_osa, ohje, malli, noudata_perusohjetta):
    if not muokattava_osa.strip():
        ...
        return None
    prompt = f"Olet kustannustoimittaja. Tehtäväsi on muokata alla olevasta KOKO TEKSTISTÄ yhtä tiettyä osiota käyttäjän ohjeen mukaan. KOKO TEKSTI (KONTEKSTIA VARTEN): --- {koko_teksti} --- MUOKATTAVA OSIO (Tämä on ainoa osa, jota saat muuttaa): --- {muokattava_osa} --- Käyttäjän muokkausohje on: \"{ohje}\". PALAUTA AINOASTAAN UUSI, PARANNELTU VERSIO MUOKATTAVASTA OSIOSTA."
    return tee_api_kutsu(prompt, malli, noudata_perusohjetta)

# --- SOVELLUKSEN KÄYTTÖLIITTYMÄ ---
st.set_page_config(page_title="Älykäs Raamattu-tutkija", layout="wide")
st.title("📖 Älykäs Raamattu-tutkija v3.4")
...
if suorita_nappi:
    if OMA_API_AVAIN == "LIITÄ_TÄHÄN_OMA_AVAIMESI": st.error("Aseta Gemini API-avain app.py-tiedostoon.")
    else:
