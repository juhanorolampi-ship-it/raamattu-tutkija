...
def laske_kustannus_arvio(token_count, model_name):
    ...
    USD_TO_EUR = 0.93
    if model_name not in HINNASTO_USD:
        ...
        for verse_num in range(ref["start_verse"], ref["end_verse"] + 1):
            for i in range(verse_num - ennen, verse_num + jalkeen + 1):
                ...
                if verse_str in chapter_data:
                    ...
                    found_verses.add(
                        f"{book_proper_name} {chapter_str}:{verse_str} - {verse_text}"
                    )
    except KeyError:
        return []
...
def etsi_ja_laajenna(
    bible_data, book_map, book_name_map, book_data_map, sana, kirja, ennen, jalkeen
):
    siemen_jakeet, sana_lower = [], sana.lower().replace("*", ".*")
    try:
        pattern = re.compile(sana_lower)
    except re.error:
        return []
    ...
    for book_id, luku, jae_nro in siemen_jakeet:
        for i in range(jae_nro - ennen, jae_nro + jalkeen + 1):
            try:
                jae_teksti = book_data_map[book_id]["chapter"][str(luku)]["verse"][
                    str(i)
                ]["text"]
...
if not st.session_state.password_correct:
    ...
    try:
        ...
        st.error(
            "API-avainta ei löydy. Määritä se Streamlitin secret-hallinnassa (GEMINI_API_KEY)."
        )
        st.stop()
    ...
    if st.session_state.step == "input":
        ...
        with st.sidebar:
            ...
            malli_valinta_ui = st.selectbox(
                "Valitse Gemini-malli:",
                ("gemini-1.5-flash", "gemini-1.5-pro"),
                index=0,
                key="malli_input",
            )
            noudata_perusohjetta_luodessa = st.checkbox(
                "Noudata teologista perusohjetta", value=True, key="noudata_input"
            )

            st.divider()
            st.session_state.show_token_counter = st.checkbox(
                "Näytä kulutuslaskurit",
                value=st.session_state.get("show_token_counter", False),
            )
            if st.session_state.show_token_counter:
                st.subheader("Session kulutus")
                session_hinta = laske_kustannus_arvio(
                    st.session_state.token_count, malli_valinta_ui
                )
                st.metric(
                    label="Tokenit",
                    value=f"{st.session_state.token_count['total']:,}",
                    help=f"Arvioidut kustannukset: {session_hinta}",
                )

                st.subheader("Päivän kulutus")
                daily_hinta = laske_kustannus_arvio(
                    st.session_state.daily_token_count, malli_valinta_ui
                )
                st.metric(
                    label="Tokenit yhteensä",
                    value=f"{st.session_state.daily_token_count['total']:,}",
                    help=f"Arvioidut kustannukset: {daily_hinta}",
                )

        aihe = st.text_area(
            "Mikä on opetuksen tai tutkimuksen pääaihe?",
            "Mitä Johannes 3:16 meille opettaa?",
            height=150,
        )
        ...
        if st.button("Luo tutkimussuunnitelma →", type="primary"):
            ...
            with st.spinner("Analysoidaan aihetta ja luodaan tutkimussuunnitelmaa..."):
                ...
                lisamateriaalit = (
                    [lue_ladattu_tiedosto(tiedosto) for tiedosto in ladatut_tiedostot]
        ...
        st.info(
            "Tekoäly on ehdottanut seuraavia hakusanoja ja Raamatun kirjoja. Voit muokata niitä vapaasti ennen kuin aloitat varsinaisen jakeiden haun."
        )
        ...
        with col1:
            hakusanat_str = st.text_area(
                "Hakusanat (yksi per rivi)",
        ...
        with col2:
            ...
            valitut_kirjat = st.multiselect(
                "Raamatun kirjat", options=kaikki_kirjat, default=default_books
            )
        ...
        if st.button("Aloita tutkimus →", type="primary"):
            with st.spinner("Kerätään aineistoa..."):
                ...
                final_hakusanat = [
                    s.strip() for s in hakusanat_str.split("\n") if s.strip()
                ]
                ...
                initial_refs = etsi_viittaukset_tekstista(
                    st.session_state.aineisto["aihe"],
                    book_map,
                    book_data_map,
                    sorted_aliases,
                )
                kaikki_loydetyt_jakeet.update(
                    v
                    for ref in initial_refs
                    for v in hae_tarkka_viittaus(
                        ref,
                        book_data_map,
                        book_name_map,
                        jakeita_ennen,
                    ...
                    for sana in final_hakusanat:
                        ...
                        if total_searches > 0:
                            progress_bar.progress(
                                ...
                                text=f"Etsitään: '{sana}' ({kirja})",
                            )

                st.session_state.aineisto["jakeet"] = sorted(
                    list(kaikki_loydetyt_jakeet)
                )

                st.write("Luodaan sisällysluettelo...")
                sisallysluettelo_str = luo_sisallysluettelo(
                    st.session_state.aineisto["aihe"],
                    st.session_state.aineisto["malli"],
                    st.session_state.aineisto["noudata_ohjetta"],
                )
        ...
        with st.sidebar:
            ...
            toimintatapa = st.radio(
                "Valitse lopputuloksen tyyppi:",
                ("Valmis opetus (Optimoitu)", "Tutkimusraportti (Jatkojalostukseen)"),
                key="toimintatapa_valinta",
            )
            sanamaara = st.number_input(
                "Tavoitesanamäärä (vain opetukselle)",
                min_value=300,
                max_value=20000,
                value=4000,
                step=100,
                key="sanamaara_valinta",
            )
            if st.button("Tarkista ja jatka →", type="primary"):
                st.session_state.aineisto["sisallysluettelo"] = (
                    st.session_state.sisallysluettelo_editori
                )
                ...
                with st.spinner("Tarkistetaan viittauksia sisällysluettelosta..."):
                    references_in_toc = etsi_viittaukset_tekstista(
                        st.session_state.sisallysluettelo_editori,
                        book_map,
                        book_data_map,
                        sorted_aliases,
                    )
                    existing_verses_list_lower = [
                        v.lower() for v in st.session_state.aineisto.get("jakeet", [])
                    ]
                    missing = [
                        ref
                        ...
                        if not all(
                            any(
                                line.startswith(
                                    f"{ref['book_name']} {ref['chapter']}:{v_num}".lower()
                                    + " -"
                                )
                                for line in existing_verses_list_lower
                            )
                            for v_num in range(ref["start_verse"], ref["end_verse"] + 1)
                        )
                    ]
                if not missing:
                    st.session_state.missing_verses = None
        ...
        if st.session_state.missing_verses:
            st.warning(
                "⚠️ **Huomio!** Seuraavia sisällysluettelossa mainittuja viittauksia ei löytynyt kerätystä aineistosta. Haluatko hakea ne ja lisätä aineistoon?"
            )
            st.markdown(
                "\n".join(
                    [
                        f"- {ref['original_match']}"
            ...
            st.text_area(
                "Muokkaa sisällysluetteloa tarvittaessa:",
                value=st.session_state.aineisto.get("sisallysluettelo", ""),
                height=300,
                key="sisallysluettelo_editori",
            )
            with st.expander(
                f"Näytä {len(st.session_state.aineisto.get('jakeet', []))} kerättyä jaetta"
            ):
                st.text_area(
                    "Kerätty lähdemateriaali:",
    ...
    # ==============================================================================
        ...
        with st.sidebar:
            ...
            if st.session_state.show_token_counter:
                st.subheader("Session kulutus")
                session_hinta_out = laske_kustannus_arvio(
                    st.session_state.token_count, st.session_state.aineisto["malli"]
                )
                st.metric(
                    label="Tokenit",
                    value=f"{st.session_state.token_count['total']:,}",
        ...
        if aineisto.get("toimintatapa") == "Valmis opetus (Optimoitu)":
            with st.status("Kirjoitetaan opetusta...", expanded=True) as status:
                sisallysluettelo_rivit = [
                    rivi.strip()
                    for rivi in aineisto["sisallysluettelo"].split("\n")
                    if rivi.strip() and re.match(r"^\d", rivi)
                ]
                koko_opetus, osioiden_maara = [], len(sisallysluettelo_rivit)
                sanamaara_per_osio = (
                    aineisto["sanamaara"] // osioiden_maara
                    if osioiden_maara > 0
                    else aineisto["sanamaara"]
                )
                ...
                status.update(
                    label="Opetus on valmis!", state="complete", expanded=False
                )
...
"""
        ...
        st.info(info_teksti)
        st.download_button(
            "Lataa tiedostona (.txt)", data=lopputulos, file_name="lopputulos.txt"
        )
